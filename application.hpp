...
namespace autodis
{
	...
	public:
		...
		void learn(std::string const& model_name, std::string const& out_file_name);
		void predict(std::string const& model_name);
		void show(std::string const& model_name);
		void show_analysis(std::string const& model_name, std::filesystem::path const& out_path);
